import "dart:isolate" as l;import "dart:html" as o;import "dart:async" as MB;import "dart:mirrors" as QC;class RC{static const  SC="Chrome";final  JC;final  minimumVersion;const RC(this.JC,[this.minimumVersion]);}class TC{const TC();}class UC{final  name;const UC(this.name);}class VC{const VC();}class WC{const WC();}var HB;main(){var t=zB(uri:"http://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20rss%20where%20url%20%3D%20%22http%3A%2F%2Fquotidianohome.feedsportal.com%2Fc%2F33327%2Ff%2F565662%2Findex.rss%22&format=json&diagnostics=true&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys&callback=?");t.then(( k){HB=new List<RB>();var h=k["query"]["results"]["item"];for(var g=0;g<h.length;g++ ){var i=new RB();i.title=h[g]["title"];i.OC=h[g]["link"];i.PC=h[g]["pubDate"];var j=h[g]["description"];var v=j.indexOf('<img')!=-1;if(v){var GB=j.indexOf('<img');i.description=j.substring(0,GB);}HB.add(i);}bB(k);print(HB.length);o.query('#rss').model=HB;});}class RB{var title,OC,description,PC;}final hB=r"""
(function() {
...
"""; iB(g){final h=new o.ScriptElement()..type='text/javascript'..text=g;o.document.body.nodes.add(h);}var DB=null;var jB=null;var kB=null;var SB=null;var lB=null;var mB=null;var nB=null;var oB=null;var TB=null;var UB=null;var VB=null;var WB=null;var XB=null;var YB=null; pB(){if(DB!=null)return;try {DB=o.window.lookupPort('dart-js-interop-context');}catch (h){}if(DB==null){iB(hB);DB=o.window.lookupPort('dart-js-interop-context');}jB=o.window.lookupPort('dart-js-interop-create');kB=o.window.lookupPort('dart-js-interop-proxy-count');SB=o.window.lookupPort('dart-js-interop-equals');lB=o.window.lookupPort('dart-js-interop-instanceof');mB=o.window.lookupPort('dart-js-interop-has-property');nB=o.window.lookupPort('dart-js-interop-delete-property');oB=o.window.lookupPort('dart-js-interop-convert');TB=o.window.lookupPort('dart-js-interop-enter-scope');UB=o.window.lookupPort('dart-js-interop-exit-scope');VB=o.window.lookupPort('dart-js-interop-globalize');WB=o.window.lookupPort('dart-js-interop-invalidate');XB=new o.ReceivePortSync()..receive((gC)=>ZB());YB=new o.ReceivePortSync()..receive((g)=>aB(g[0]));o.window.registerPort('js-dart-interop-enter-scope',XB.toSendPort());o.window.registerPort('js-dart-interop-exit-scope',YB.toSendPort());} get qB{NB();return JB(DB.callSync([] ));}get rB=>m.hC.length; NB(){if(rB==0){var g=ZB();MB.runAsync(()=>aB(g));}} ZB(){pB();m.KC();TB.callSync([] );return m.hC.length;} aB( g){assert(m.hC.length==g);UB.callSync([] );m.LC();} sB( g){VB.callSync(BB(g.FB()));return g;} bB( g){WB.callSync(BB(g.FB()));}class XC implements CB<IB>{var iC;var jC;var kC;pB(g){iC=g;jC=m.add(kC);m.MC(jC);}lC(){var g=m.NC(jC);} FB()=>new IB.fC(m.LB,jC);XC.eC( h,{ withThis: false}){kC=( g){try {return Function.apply(h,withThis?g:g.skip(1).toList());}finally {lC();}};pB(false);}}class YC{const YC();}const AB=const YC(); tB(i,j,k,t,v,GB){final g=[i,j,k,t,v,GB];final h=g.indexOf(AB);if(h<0)return g;return g.sublist(0,h);}class q implements CB<q>{var mC;final jC;q.fC(this.mC,this.jC); FB()=>this;operator[](g)=>EB(this,'[]','method',[g]);operator[]=(g,h)=>EB(this,'[]=','method',[g,h]);operator==(g)=>identical(this,g)?true:(g is q&&SB.callSync([BB(this),BB(g)])); toString()=>EB(this,'toString','method',[] ,onNone:()=>super.toString());noSuchMethod( h){var g=QC.MirrorSystem.getName(h.memberName);if(g.indexOf('@')!=-1){g=g.substring(0,g.indexOf('@'));}var i;var j=h.positionalArguments;if(j==null)j=[] ;if(h.isGetter){i='get';}else if(h.isSetter){i='set';if(g.endsWith('=')){g=g.substring(0,g.length-1);}}else if(g=='call'){i='apply';}else{i='method';}return EB(this,g,i,j,onNone:()=>super.noSuchMethod(h));}static EB( h, i, j, k,{onNone()}){NB();var g=h.mC.callSync([h.jC,i,j,k.map(BB).toList()]);switch (g[0]){case 'return':return JB(g[1]);case 'throws':throw JB(g[1]);case 'none':return onNone==null?null:onNone();default:throw 'Invalid return value';}}}class IB extends q implements CB<IB>{IB.fC( g,h):super.fC(g,h);call([g=AB,h=AB,i=AB,j=AB,k=AB,t=AB]){var v=tB(g,h,i,j,k,t);return q.EB(this,'','apply',v);}}abstract class CB<uB>{ FB();}class vB{final  nC;var oC;var pC;final  qC;final  mC;final  rC;final  sC;final  hC;KC(){hC.add(sC.length);}LC(){var h=hC.removeLast();for(int g=h;g<sC.length; ++g){var i=sC[g];if(!rC.contains(i)){qC.remove(sC[g]);pC++ ;}}if(h!=sC.length){sC.removeRange(h,sC.length);}}MC(g)=>rC.add(g);NC(g){var h=qC[g];rC.remove(g);qC.remove(g);pC++ ;return h;}vB():nC='dart-ref',oC=0,pC=0,qC={},mC=new o.ReceivePortSync(),sC=new List<String>(),hC=new List<int>(),rC=new Set<String>(){mC.receive((g){try {final h=qC[g[0]];final i=g[1];final j=g[2].map(JB).toList();if(i=='#call'){final k=h as Function;var t=BB(k(j));return ['return',t];}else{throw 'Invocation unsupported on non-function Dart proxies';}}catch (v){return ['throws','${v}'];}});} add(h){NB();final g='${nC}-${oC++ }';qC[g]=h;sC.add(g);return g;}Object get( g){return qC[g];}get LB=>mC.toSendPort();}var m=new vB();BB(var g){if(g==null){return null;}else if(g is String||g is num||g is bool){return g;}else if(g is l.SendPortSync){return g;}else if(g is o.Element&&(g.document==null||g.document==o.document)){return ['domref',xB(g)];}else if(g is IB){return ['funcref',g.jC,g.mC];}else if(g is q){return ['objref',g.jC,g.mC];}else if(g is CB){return BB(g.FB());}else{return ['objref',m.add(g),m.LB];}}JB(var g){j(g){var h=g[1];var i=g[2];if(i==m.LB){return m.get(h);}else{return new IB.fC(i,h);}}k(g){var h=g[1];var i=g[2];if(i==m.LB){return m.get(h);}else{return new q.fC(i,h);}}if(g==null){return null;}else if(g is String||g is num||g is bool){return g;}else if(g is l.SendPortSync){return g;}var t=g[0];switch (t){case 'funcref':return j(g);case 'objref':return k(g);case 'domref':return yB(g[1]);}throw 'Unsupported serialized data: ${g}';}var wB=0;const KB='data-dart_id';const u='data-dart_temporary_attached';xB( h){var i;if(h.attributes.containsKey(KB)){i=h.attributes[KB];}else{i='dart-${wB++ }';h.attributes[KB]=i;}if(!identical(h,o.document.documentElement)){var g=h;while (true){if(g.attributes.containsKey(u)){final j=g.attributes[u];final k=j+'a';g.attributes[u]=k;break;}if(g.parent==null){g.attributes[u]='a';o.document.documentElement.children.add(g);break;}if(identical(g.parent,o.document.documentElement)){break;}g=g.parent;}}return i;} yB(var h){var i=o.queryAll('[${KB}="${h}"]');if(i.length>1)throw 'Non unique ID: ${h}';if(i.length==0){throw 'Only elements attached to document can be serialized: ${h}';}final j=i[0];if(!identical(j,o.document.documentElement)){var g=j;while (true){if(g.attributes.containsKey(u)){final k=g.attributes[u];final t=k.substring(1);g.attributes[u]=t;if(g.attributes[u].length==0){g.attributes.remove(u);g.remove();}break;}if(identical(g.parent,o.document.documentElement)){break;}g=g.parent;}}return j;} zB({ uri: null, uriGenerator( callback): null, type: null})=>GC(const OB(const ZC(),const aC()),uri:uri,uriGenerator:uriGenerator,type:type);class ZC extends cB{const ZC(); fB( h, i){qB[h]=new XC.eC(( g){sB(g);i.complete(g);});} gB( g){bB(g);}}class aC extends dB{const aC(); request( g){o.document.body.nodes.add(new o.ScriptElement()..src=g);}}abstract class cB{const cB(); fB( g, h); gB(var g);}abstract class dB{const dB(); request( g);}class OB<AC extends cB,BC extends dB>{final  QB;final  html;const OB(this.QB,this.html);}class CC{static var DC=0;static  EC(){return "jsonp_receive_${DC++ }";}final  PB;final  eB=EC();CC(this.PB); request( g( callback))=>PB.html.request(g(eB)); convert( h,var g){var i=QC.reflectClass(h).newInstance(const Symbol('fromProxy'),[g]);PB.QB.gB(g);return i.reflectee;}}class FC extends CC{final  tC=new MB.Completer();FC( g):super(g){g.QB.fB(eB,tC);} future({ type: null})=>type==null?tC.future:tC.future.then((var g)=>convert(type,g));} GC( h,{ uri: null, uriGenerator( callback): null, type: null}){try {final  g=new FC(h);g.request(( i)=>HC(uri,uriGenerator,i));return g.future(type:type);}catch (j){return new MB.Future.error(j);}} HC( g, h( callback), i){if(g==null&&h==null){throw new ArgumentError("Missing Parameter: uri or uriGenerator required");}return g!=null?IC(g,i):h(i);} IC( v, GB){var g,i;var h;var j=0;g=Uri.parse(v);h=new Map<String,String>();g.queryParameters.forEach(( k, t){if(t=='?'){h[k]=GB;j++ ;}else{h[k]=t;}});if(j==0){throw new ArgumentError("Missing Callback Placeholder: when providing a uri, at least one query parameter must have the ? value");}i=new Uri(scheme:g.scheme,userInfo:g.userInfo,host:g.host,port:g.port,path:g.path,fragment:g.fragment,queryParameters:h);return i.toString();}const bC=const cC();class cC{const cC();}